...
Next, the number of orders can be further divided by customers' segments. The following query is used:
...
Last but not least, the profits generated by each manager is analyzed. The following basic query is used:
...
Interestingly, the result shows that the profits generated does not proportional to the number of orders handled.
